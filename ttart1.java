...
public class ttart1 extends HttpServlet
{
	...
	try
	{
		...
		String qry="select title,type,artist1,artist2,s1,s2,rate from music where artist1 ='"+ alph +"' or artist2='"+alph+"'";
		pw.println(qry);
		...
		pw.println("<pre><label>		Item name:        </label><input type=text name=txtr></pre>");pw.println("<font size=3><br><br><input type=submit value=Add_to_Cart></font></form>");
		pw.println("<pre><label> 		Please enter H/h for Hardware  S/s for Software M/m for Music   B/b for Books</pre></label>");
		pw.println("<pre><input type=submit value=Add_to_cart>");
}
